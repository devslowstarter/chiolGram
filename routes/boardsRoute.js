...
router.post('/board', authMiddleware, async (req, res) => {
  ...
  const { title, content, hashtag, image } = req.body;
  ...
  try {
    ...
    console.error(error);
    return res.status(500).json({ message: '게시글 등록 과정에 오류가 발생하였습니다.' });
  }
});
...
router.get('/board', async (req, res) => {
  try {
    ...
    console.error(error);
    return res.status(500).json({ message: '게시글 전제 조회 과정에 오류가 발생하였습니다.' });
  }
});
...
router.put('/board/:boardId', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (!boards) {
      return res.status(404).json({ errorMessage: '게시글를 찾을 수 없습니다.' });
    }
    ...
    console.error(error);
    return res.status(500).json({ errorMessage: '게시글 수정 과정에 오류가 발생하였습니다.' });
  }
});
